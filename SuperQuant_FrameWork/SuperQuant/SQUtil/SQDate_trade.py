...
def SQ_util_if_tradetime(_time=datetime.datetime.now(), market=MARKET_TYPE.STOCK_CN, code=None):
    '时间是否交易'
    _time = datetime.datetime.strptime(str(_time)[0:19], '%Y-%m-%d %H:%M:%S')
    if market is MARKET_TYPE.STOCK_CN:
        if SQ_util_if_trade(str(_time.date())[0:10]):
...
def SQ_util_get_next_day(date, n=1):
    '''
    得到下一个(n)交易日
    ...
    '''
    date = str(date)[0:10]
    return SQ_util_date_gap(date, n, 'gt')


def SQ_util_get_last_day(date, n=1):
    '''
    得到上一个(n)交易日
    :param  date: 类型 str eg: 2018-11-11
    :param n:  整形
    ...
    '''
    date = str(date)[0:10]
    return SQ_util_date_gap(date, n, 'lt')


def SQ_util_get_last_datetime(datetime, day=1):
    date = str(datetime)[0:10]
    return '{} {}'.format(SQ_util_date_gap(date, day, 'lt'), str(datetime)[11:])


def SQ_util_get_next_datetime(datetime, day=1):
    date = str(datetime)[0:10]
    return '{} {}'.format(SQ_util_date_gap(date, day, 'gt'), str(datetime)[11:])


def SQ_util_get_real_date(date, trade_list=trade_date_sse, towards=-1):
    """
...
def SQ_util_get_real_datelist(start, end):
    """
    取数据的真实区间,返回的时候用 start,end=SQ_util_get_real_datelist
    @yutiansut
    2017/8/10
    ...
    """
    real_start = SQ_util_get_real_date(start, trade_date_sse, 1)
    real_end = SQ_util_get_real_date(end, trade_date_sse, -1)
...
def SQ_util_get_trade_range(start, end):
    '给出交易具体时间'
    start, end = SQ_util_get_real_datelist(start, end)
...
def SQ_util_get_trade_gap(start, end):
...
def SQ_util_date_gap(date, gap, methods):
    '''
...
def SQ_util_get_trade_datetime(dt=datetime.datetime.now()):
    """交易的真实日期
    ...
    if SQ_util_if_trade(str(dt.date())) and dt.time() < datetime.time(15, 0, 0):
        return str(dt.date())
    else:
        return SQ_util_get_real_date(str(dt.date()), trade_date_sse, 1)


def SQ_util_get_order_datetime(dt):
    """委托的真实日期
    ...
    if SQ_util_if_trade(str(dt.date())) and dt.time() < datetime.time(15, 0, 0):
        ...
        # print('before')
        # print(SQ_util_date_gap(str(dt.date()),1,'lt'))
        return '{} {}'.format(SQ_util_date_gap(str(dt.date()), 1, 'lt'), dt.time())


def SQ_util_future_to_tradedatetime(real_datetime):
    ...
    if len(str(real_datetime)) == 19:
        ...
        return dt if dt.time() < datetime.time(21, 0) else SQ_util_get_next_datetime(dt, 1)
    elif len(str(real_datetime)) == 16:
        dt = datetime.datetime.strptime(str(real_datetime)[0:16], '%Y-%m-%d %H:%M')
        return dt if dt.time() < datetime.time(21, 0) else SQ_util_get_next_datetime(dt, 1)


def SQ_util_future_to_realdatetime(trade_datetime):
    ...
    if len(str(trade_datetime)) == 19:
        ...
        return dt if dt.time() < datetime.time(21, 0) else SQ_util_get_last_datetime(dt, 1)
