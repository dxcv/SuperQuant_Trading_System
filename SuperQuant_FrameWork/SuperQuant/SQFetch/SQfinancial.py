...
class SQHistoryFinancialReader(HistoryFinancialReader):
    ...
    def get_df(self, data_file):
        ...
        crawler = SQHistoryFinancialCrawler()
...
def get_filename():
    ...
    get_filename
    """
    return [(l[0], l[1]) for l in
            [line.strip().split(",") for line in requests.get(FINANCIAL_URL).text.strip().split('\n')]]
...
def get_and_parse(filename):
    return SQHistoryFinancialReader().get_df(filename)
