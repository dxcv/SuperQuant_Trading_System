...
from SuperQuant.SQFetch.base import headers
...
def SQ_fetch_get_stock_day_in_year(code, year, if_fq='00'):
    data_ = []
...
def SQ_fetch_get_stock_day(code, start, end, if_fq='00'):
    start_year = int(str(start)[0:4])
    end_year = int(str(end)[0:4])
    data = SQ_fetch_get_stock_day_in_year(code, start_year, if_fq)
    if start_year < end_year:
        for i2 in range(start_year + 1, end_year + 1):
            data = pd.concat(
                [data, SQ_fetch_get_stock_day_in_year(code, i2, if_fq)], axis=0)
    else:
        pass
    if data is None:
        return pd.DataFrame()
...
def SQ_fetch_get_stock_block():
    ...
    url = 'https://gitee.com/yutiansut/SQDATA/raw/master/ths_block.csv'
    try:
...
def SQ_fetch_get_stock_highlimit_reason(code):
...
if __name__ == '__main__':
    ...
    print(SQ_fetch_get_stock_day('000001', '2016-05-01', '2017-07-01', '01'))
