...
def SQ_fetch_get_option_contract_time_to_market():
    '''
    ...
    C:\work_new\SuperQuant\SuperQuant\SQFetch\SQTdx.py:1468: SettingWithCopyWarning: 
...
def SQ_fetch_get_option_50etf_contract_time_to_market():
    '''
        #🛠todo 获取期权合约的上市日期 ？ 暂时没有。
        :return: list Series
        '''
...
def SQ_fetch_get_commodity_option_CF_contract_time_to_market():
    '''
    铜期权  CU 开头   上期证
    豆粕    M开头     大商所
    白糖    SR开头    郑商所
...
def SQ_fetch_get_commodity_option_RU_contract_time_to_market():
    '''
    铜期权  CU 开头   上期证
    豆粕    M开头     大商所
    白糖    SR开头    郑商所
...
def SQ_fetch_get_commodity_option_C_contract_time_to_market():
    '''
    铜期权  CU 开头   上期证
    豆粕    M开头     大商所
    白糖    SR开头    郑商所
...
def SQ_fetch_get_future_day(code, start_date, end_date, frequence='day', ip=None, port=None):
    ...
    with apix.connect(ip, port):
        ...
        try:
            ...
            # 获取商品期货会报None
            data = data.assign(date=data['datetime'].apply(lambda x: str(x[0:10]))).assign(code=str(code)) \
                .assign(date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(str(x)[0:10]))).set_index('date',
                                                                                                                 drop=False,
                                                                                                                 inplace=False)
...
def SQ_fetch_get_future_transaction(code, start, end, retry=4, ip=None, port=None):
    '期货历史成交分笔'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    global extension_market_list
