...
class SQ_Market(SQ_Trade):
    ...
    def get_account(self, account_cookie):
        try:
            ...
            print(
                'SQMARKET: this account {} is logoff, please login and retry'
    ...
    def on_insert_order(self, order: SQ_Order):
        print(order)
        print(order.status)
    ...
    def _renew_account(self):
        for account in self.session.values():
            account.run(SQ_Event(event_type=ACCOUNT_EVENT.SETTLE))
    ...
    def _settle(self, broker_name, callback=False):
        ...
        self._trade(event=SQ_Event(broker_name=broker_name))
