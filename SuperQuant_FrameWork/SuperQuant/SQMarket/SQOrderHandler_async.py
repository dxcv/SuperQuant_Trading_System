...
class SQ_OrderHandlerAsync(SQ_Worker):
    """ORDER执行器
    异步handler
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.order_queue = SQ_OrderQueue()
    ...
    def run(self, event):
        if event.event_type is BROKER_EVENT.RECEIVE_ORDER:
            ...
            实盘涉及到外部订单问题: 
            及 订单的来源 不完全从SuperQuant中发出, 则SQ无法记录来源 (标记为外部订单)
            """

            if self.if_start_orderquery:
                # if SQ_util_if_tradetime(datetime.datetime.now()):
                ...
                if len(self.order_status) > 0:
                    SQ_SU_save_order(self.order_status)
                # else:
                #     time.sleep(1)
            ...
            if self.if_start_orderquery:
                ...
                if len(self.deal_status) > 0:
                    SQ_SU_save_deal(self.deal_status)
                # print(self.order_status)
