...
from SuperQuant.SQFetch.SQQuery import (SQ_fetch_future_day,
                                        SQ_fetch_future_min, SQ_fetch_index_day,
                                        SQ_fetch_index_min, SQ_fetch_stock_day,
                                        SQ_fetch_stock_min)
...
class SQ_BacktestBroker(SQ_Broker):
    """
    ...
    def __init__(self, if_nondatabase=False):
        ...
        self.fetcher = {(MARKET_TYPE.STOCK_CN, FREQUENCE.DAY): SQ_fetch_stock_day,
                        (MARKET_TYPE.STOCK_CN, FREQUENCE.FIFTEEN_MIN): SQ_fetch_stock_min,
                        (MARKET_TYPE.STOCK_CN, FREQUENCE.ONE_MIN): SQ_fetch_stock_min,
                        (MARKET_TYPE.STOCK_CN, FREQUENCE.FIVE_MIN): SQ_fetch_stock_min,
                        (MARKET_TYPE.STOCK_CN, FREQUENCE.THIRTY_MIN): SQ_fetch_stock_min,
                        (MARKET_TYPE.STOCK_CN, FREQUENCE.SIXTY_MIN): SQ_fetch_stock_min,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.DAY): SQ_fetch_future_day,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.FIFTEEN_MIN): SQ_fetch_future_min,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.ONE_MIN): SQ_fetch_future_min,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.FIVE_MIN): SQ_fetch_future_min,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.THIRTY_MIN): SQ_fetch_future_min,
                        (MARKET_TYPE.FUTURE_CN, FREQUENCE.SIXTY_MIN): SQ_fetch_future_min,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.DAY): SQ_fetch_index_day,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.FIFTEEN_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.ONE_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.FIVE_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.THIRTY_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.INDEX_CN, FREQUENCE.SIXTY_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.DAY): SQ_fetch_index_day,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.FIFTEEN_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.ONE_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.FIVE_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.THIRTY_MIN): SQ_fetch_index_min,
                        (MARKET_TYPE.FUND_CN, FREQUENCE.SIXTY_MIN): SQ_fetch_index_min}
    ...
    def run(self, event):
        # strDbg = SQ_util_random_with_topic("SQBacktestBroker.run")
        # print("         >-----------------------SQBacktestBroker.run----------------------------->", strDbg,'evt->', event)
        ...
        if event.event_type is MARKET_EVENT.QUERY_DATA:
            ...
            self.order_handler.run(event)
            # self.run(SQ_Event(event_type=BROKER_EVENT.TRADE, broker=self))
    ...
    def receive_order(self, event):
        ...
        if 'market_data' in event.__dict__.keys():
            ...
            if isinstance(self.market_data, dict):
                ...
                self.market_data = SQ_util_to_json_from_pandas(self.market_data)[
                    0]
    ...
    def query_orders(self, account, status=''):
        ...
        if status == '':
            return self.dealer.deal_df.query('account_cookie=="{}"'.format(account)).loc[:,
                   self.orderstatus_headers].set_index(['account_cookie', 'realorder_id'])
        elif status == 'filled':
            return self.dealer.deal_df.query('account_cookie=="{}"'.format(account)).loc[:,
                   self.dealstatus_headers].set_index(['account_cookie', 'realorder_id'])
        elif status == 'open':
            pass
    ...
    def warp(self, order):
        ...
        if order.order_model == ORDER_MODEL.MARKET:
            ...
            # try:
            #     order.date = SQ_util_get_next_day(str(order.datetime)[0:10])
            #     order.datetime = '{} 09:30:00'.format(order.date)
            # except:
        ...
        if (pd.Timestamp(order.datetime), order.code) in self._quotation.keys():
            ...
            try:
                data = self.fetcher[(order.market_type, order.frequence)](
                    code=order.code, start=order.datetime, end=order.datetime, format='json',
                    frequence=order.frequence)[0]
