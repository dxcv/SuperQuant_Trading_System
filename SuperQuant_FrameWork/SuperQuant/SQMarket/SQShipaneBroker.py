...
from SuperQuant.SQMarket.common import (cn_en_compare, order_status_cn_en,
                                       trade_towards_cn_en)
...
class SQ_SPEBroker(SQ_Broker):
    ...
    receive_order(SQMARKET 用法):
    输入一个SQ_ORDER类

    如果下单成功 返回带realorder_id, ORDER_STATUS.QUEUED状态值 的SQ_Order
    如果下单失败 返回带 ORDER_STATUS.FAILED状态值的  SQ_Order
                ...
                if 'order_time' in order_headers:
                    ...
                    if 'order_date' not in order_headers:
                        order_all.order_time = order_all.order_time.apply(
                            lambda x: SQ_util_get_order_datetime(dt='{} {}'.format(datetime.date.today(), x)))
                    else:
                        order_all = order_all.assign(order_time=order_all.order_date.apply(
                            SQ_util_date_int2str)+' '+order_all.order_time)
