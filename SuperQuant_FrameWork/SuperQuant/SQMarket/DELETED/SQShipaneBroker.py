...
def get_config_SPE():
    ...
    return SPE_CONFIG(
        SQ_Setting().get_config('SPE', 'uri', DEFAULT_SHIPANE_URL),
        SQ_Setting().get_config('SPE', 'key', DEFAULT_SHIPANE_KEY)
    )
                ...
                if 'order_time' in order_headers:
                    ...
                    if 'order_date' not in order_headers:
                        order_all.order_time = order_all.order_time.apply(
                            lambda x: SQ_util_get_order_datetime(
