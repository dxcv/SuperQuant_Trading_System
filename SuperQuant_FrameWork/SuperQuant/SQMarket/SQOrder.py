...
class SQ_Order():
    ...
    def to_otgdict(self):
        """{
                ...
                "order_id": order_id if order_id else SQ.SQ_util_random_with_topic('SQOTG'),
    ...
    def from_dict(self, order_dict):
        ...
        :return: self SQ_Order
        '''
        try:
            # SQ_util_log_info('SQ_ORDER CHANGE: from {} change to {}'.format(
            ...
            SQ_util_log_info('Failed to tran from dict {}'.format(e))
...
class SQ_OrderQueue():  # also the order tree ？？ what's the tree means?
    ...
    def __repr__(self):
        ...
        # return '< SQ_OrderQueue AMOUNT {} WAITING TRADE {} >'.format(len(self.queue_df), len(self.pending))
    ...
    def insert_order(self, order):
        '''
        :param order: SQ_Order类型
        :return:
        '''
        # print("     *>> SQOrder!insert_order  {}".format(order))
        # QUEUED = 300  # queued 用于表示在order_queue中 实际表达的意思是订单存活 待成交
        ...
        if order is not None:
            ...
            print('SQERROR Wrong for get None type while insert order to Queue')
