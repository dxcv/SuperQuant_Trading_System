...
class SQ_Dealer():
    ...
    def callback_message(self):
        ...
        return [self.order.account_cookie, self.order.sending_time, self.trade_time, self.order.code, None, self.order.towards, float("%.2f" % float(self.deal_price)),
                self.order.price, self.status, self.order.amount, self.deal_amount, self.trade_money, 0,  self.order.order_id, SQ_util_random_with_topic('Trade')]
        # self.order.
    ...
    def backtest_dealer(self):
        ...
        try:
            ...
            SQ_util_log_info('MARKET ENGINE ERROR: {}'.format(e))
