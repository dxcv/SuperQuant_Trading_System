...
class _quotation_base():
    ...
    🥑index_day  字符串 初始化  👤👥SQ_DataStruct_Index_day继承
    🥑index_min  字符串 初始化  👤👥SQ_DataStruct_Index_min继承
    🥑stock_day  字符串 初始化  👤👥SQ_DataStruct_Stock_day继承
    🥑stock_min  字符串 初始化  👤👥SQ_DataStruct_Stock_min继承
    🥑future_min 字符串 初始化  👤👥SQ_DataStruct_Future_min继承
    🥑future_day 字符串 初始化  👤👥SQ_DataStruct_Future_day继承
    '''
    ...
    def __init__(self, DataFrame, dtype='undefined', if_fq='bfq', marketdata_type='None'):
        ...
        self.type = dtype
        self.data_id = SQ_util_random_with_topic('DATA', lens=3)

        # 默认是不复权
        self.if_fq = if_fq
        # dtype 参数 指定类 mongo 中 collection 的名字   ，
        # 🛠todo 检查 dtype 字符串是否合法， 放到抽象类中，用子类指定数据库， 后期可以支持mongodb分片集群
        # 🛠todo 子类中没有用到mongodb的数据是通过， SQ_data_stock_to_fq  实现数据复权的
    ...
    def __repr__(self):
        return '< SQ_Base_DataStruct with %d securities >' % len(self.code)
    ...
    def __reversed__(self):
        ...
        raise NotImplementedError(
            'SQ_DataStruct_* CURRENT CURRENTLY NOT SUPPORT reversed ACTION')

    def __add__(self, DataStruct):
        '''
        ➕合并数据，重复的数据drop
        :param DataStruct: _quotation_base 继承的子类  SQ_DataStruct_XXXX
        :return: _quotation_base 继承的子类  SQ_DataStruct_XXXX
        '''
        assert isinstance(DataStruct, _quotation_base)
        assert self.is_same(DataStruct)
        # 🛠todo 继承的子类  SQ_DataStruct_XXXX 类型的 判断必须是同一种类型才可以操作
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            'SQ_DataStruct_* Class Currently has no attribute {}'.format(attr))
    ...
    def get_dict(self, time, code):
        ...
        '''
        try:
            return self.dicts[(SQ_util_to_datetime(time), str(code))]
        except Exception as e:
            raise e
    ...
    def kline_echarts(self, code=None):
        ...
        if code is None:
            path_name = '.' + os.sep + 'SQ_' + self.type + \
            ...
            grid = Grid(width=1360, height=700, page_title='SuperQuant')
            grid.add(bar, grid_top="80%")
            grid.add(kline, grid_bottom="30%")
            return grid

    def plot(self, code=None):
        path_name = '.{}SQ_{}_{}_{}.html'.format(
    ...
    def get(self, name):
        ...
        if name in self.data.__dir__():
            ...
            raise ValueError('SQDATASTRUCT CANNOT GET THIS PROPERTY')
    ...
    def query(self, context):
        ...
        try:
            ...
            print('SQ CANNOT QUERY THIS {}'.format(context))
            pass
    ...
    def reindex(self, ind):
        ...
        if isinstance(ind, pd.MultiIndex):
            try:
                ...
                raise RuntimeError('SQDATASTRUCT ERROR: CANNOT REINDEX')
        else:
            raise RuntimeError(
                'SQDATASTRUCT ERROR: ONLY ACCEPT MULTI-INDEX FORMAT')
    ...
    def reindex_time(self, ind):
        if isinstance(ind, pd.DatetimeIndex):
            ...
            raise RuntimeError(
                'SQDATASTRUCT ERROR: ONLY ACCEPT DATETIME-INDEX FORMAT')
    ...
    def show(self):
        ...
        打印数据包的内容
        """
        return SQ_util_log_info(self.data)
    ...
    def to_json(self):
        ...
        转换DataStruct为json
        """
        return SQ_util_to_json_from_pandas(self.data.reset_index())
    ...
    def selects(self, code, start, end=None):
        ...
        try:
            ...
            raise ValueError(
                'SQ CANNOT GET THIS CODE {}/START {}/END{} '.format(code, start, end))
    ...
    def select_time(self, start, end=None):
        ...
        try:
            ...
            raise ValueError(
                'SQ CANNOT GET THIS START {}/END{} '.format(start, end))
    ...
    def select_day(self, day):
        ...
        try:
            ...
            raise ValueError('SQ CANNOT GET THIS Day {} '.format(day))
    ...
    def select_month(self, month):
        ...
        try:
            ...
            raise ValueError('SQ CANNOT GET THIS Month {} '.format(month))
    ...
    def select_code(self, code):
        ...
        try:
            ...
            raise ValueError('SQ CANNOT FIND THIS CODE {}'.format(code))
    ...
    def select_time_with_gap(self, time, gap, method):
        ...
        if method in ['gt', '>']:
            ...
            raise ValueError(
                'SQ CURRENTLY DONOT HAVE THIS METHODS {}'.format(method))
