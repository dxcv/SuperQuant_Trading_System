...
class SQ_DataStruct_Stock_day(_quotation_base):
    ...
    def __init__(self, init_data_by_df, dtype='stock_day', if_fq='bfq'):
        '''
        # 🛠 todo dtype=stock_day 和 SQ_DataStruct_Stock_day 类的名字是对应的 不变的不需要指定 ，容易出错，建议改成常量 ❌
        ...
        if isinstance(init_data_by_df, pd.DataFrame) == False:
            print("SQError init_data_by_df is not kind of DataFrame type !")
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Stock_day with {} securities >'.format(len(self.code))
    __str__ = __repr__
    ...
    def to_qfq(self):
        if self.if_fq is 'bfq':
            ...
            #         lambda x: SQ_data_stock_to_fq(self.data[self.data['code'] == x]), self.code))), self.type, 'qfq')
            else:
                return self.new(
                    self.groupby(level=1).apply(SQ_data_stock_to_fq, 'qfq'), self.type, 'qfq')
        else:
            SQ_util_log_info(
    ...
    def to_hfq(self):
        if self.if_fq is 'bfq':
            if len(self.code) < 1:
                ...
                return self.new(
                    self.groupby(level=1).apply(SQ_data_stock_to_fq, 'hfq'), self.type, 'hfq')
                # return self.new(pd.concat(list(map(lambda x: SQ_data_stock_to_fq(
    ...
    def resample(self, level):
        try:
            return self.add_func(SQ_data_day_resample, level).sort_index()
        except Exception as e:
            print('SQ ERROR : FAIL TO RESAMPLE {}'.format(e))
            return None


class SQ_DataStruct_Stock_min(_quotation_base):
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Stock_Min with {} securities >'.format(len(self.code))
    __str__ = __repr__
    ...
    def to_qfq(self):
        if self.if_fq is 'bfq':
            if len(self.code) < 1:
                ...
                return self
            # elif len(self.code) < 20:
            #     data = SQ_DataStruct_Stock_min(pd.concat(list(map(lambda x: SQ_data_stock_to_fq(
    ...
    def resample(self, level):
        try:
            return self.add_func(SQ_data_min_resample, level).sort_index()
        except Exception as e:
            print('SQ ERROR : FAIL TO RESAMPLE {}'.format(e))
            return None
...
class SQ_DataStruct_Future_day(_quotation_base):
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Future_day with {} securities >'.format(len(self.code))
    __str__ = __repr__
...
class SQ_DataStruct_Future_min(_quotation_base):
    """
    struct for future
    """
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Future_min with {} securities >'.format(len(self.code))
    __str__ = __repr__
...
class SQ_DataStruct_Index_day(_quotation_base):
    '自定义的日线数据结构'
    ...
    def __add__(self,DataStruct):
        ...
        assert isinstance(DataStruct,SQ_DataStruct_Index_day)
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Index_day with {} securities >'.format(len(self.code))
    __str__ = __repr__
...
class SQ_DataStruct_Index_min(_quotation_base):
    '自定义的分钟线数据结构'
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Index_Min with %s securities >' % len(self.code)
...
class SQ_DataStruct_Stock_transaction():
    ...
    def __repr__(self):
        return '< SQ_DataStruct_Stock_Transaction >'
    ...
    def resample(self, type_='1min'):
        ...
        return SQ_DataStruct_Stock_min(SQ_data_tick_resample(self.data, type_))
...
class SQ_DataStruct_Day(_quotation_base):
...
class SQ_DataStruct_Min(_quotation_base):
    '''这个类是个通用类 一般不使用  特定生成的时候可能会用到 只具备基类方法
    '''
...
class _realtime_base():
    ...
    def __init__(self, market_data):
        ...
        if isinstance(market_data, dict):
            ...
            self.data = SQ_util_to_json_from_pandas(market_data)
...
class SQ_DataStruct_Stock_realtime(_realtime_base):
    def __init__(self, data):
        self.data = data

    def __repr__(self):
        return '< SQ_REALTIME_STRUCT code {} start {} end {} >'.format(self.code.unique(), self.datetime.iloc[1], self.datetime.iloc[-1])
    ...
    def resample(self, level):
        return SQ_data_tick_resample(self.data, level)


class SQ_DataStruct_Stock_realtime_series():
    def __init__(self, sr_series):

        if isinstance(sr_series[0], SQ_DataStruct_Stock_realtime):
            self.sr_series = sr_series
        elif isinstance(sr_series[0], dict):
            self.sr_series = [
                SQ_DataStruct_Stock_realtime(sr) for sr in sr_series]
        self.table = pd.concat([sr._data for sr in self.sr_series])


class SQ_DataStruct_Security_list():
    def __init__(self, DataFrame):
...
class SQ_DataStruct_Future_tick():
    """
    CTP FORMAT
    {'TradingDay': '20181115',
    'InstrumentID': 'rb1901',
