...
from SuperQuant.SQSetting.SQSetting import SQ_Setting, DATABASE
from SuperQuant.SQUtil.SQDate_trade import SQ_util_get_next_day, SQ_util_get_real_date
...
class SQ_User():
    ...
    def __repr__(self):
        return '< SQ_USER {} with {} portfolio: {} >'.format(
            self.user_cookie,
        ...
        if self.coins > cost_coins:
            ...
            self._subscribed_strategy[strategy_id] = {
                ...
                SQ_util_get_next_day(
                    SQ_util_get_real_date(str(today),
                                          towards=1),
                    last
                ),
                'status':
