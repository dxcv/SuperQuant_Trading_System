...
from SuperQuant.SQFetch.SQQuery import SQ_fetch_stock_day
from SuperQuant.SQUtil import SQ_util_log_info, trade_date_sse


def SQ_backtest_analysis_backtest(client, code_list, assets_d, account_days, message, total_date, benchmark_data):
    ...
    if (len(benchmark_data)) < 1:
        SQ_util_log_info('Wrong with benchmark data ! ')
        sys.exit()
    ...
    # benchmark资产
    benchmark_assets = SQ_backtest_calc_benchmark(
        benchmark_data, assets[0])
    # d2=pd.concat([data.resample('D').last(),pd.DataFrame(benchmark_assets,columns=['benchmark'])])
    # benchmark年化收益
    benchmark_annualized_returns = SQ_backtest_calc_profit_per_year(
        benchmark_assets, len(total_date))
    # 计算账户的收益

    # days=len(assest_history)-1
    # 策略年化收益
    annualized_returns = SQ_backtest_calc_profit_per_year(
        assets_d, len(total_date))

    # 收益矩阵
    assest_profit = SQ_backtest_calc_profit_matrix(assets)
    benchmark_profit = SQ_backtest_calc_profit_matrix(benchmark_assets)

    # 策略日收益
    profit_day = SQ_backtest_calc_profit_matrix(assets_d)
    # 胜率
    win_rate = SQ_backtest_calc_win_rate(assest_profit)
    # 日胜率
    win_rate_day = SQ_backtest_calc_win_rate(profit_day)
    # 年化波动率
    volatility_year = SQ_backtest_calc_volatility(profit_day)
    benchmark_volatility_year = SQ_backtest_calc_volatility(benchmark_profit)
    # 夏普比率
    sharpe = SQ_backtest_calc_sharpe(
        annualized_returns, 0.05, volatility_year)

    # 最大回撤
    max_drop = SQ_backtest_calc_dropback_max(assets_d)

    # 计算beta
    beta = SQ_backtest_calc_beta(profit_day, benchmark_profit)
    # 计算Alpha
    alpha = SQ_backtest_calc_alpha(
...
def SQ_backtest_calc_assets(trade_history, assets):
    assets_d = []
    trade_date = []
...
def SQ_backtest_calc_benchmark(benchmark_data, init_assets):

    return list(benchmark_data['close'] / float(benchmark_data['open'][0]) * float(init_assets))


def SQ_backtest_calc_alpha(annualized_returns, benchmark_annualized_returns, beta, r):
...
def SQ_backtest_calc_beta(assest_profit, benchmark_profit):
...
def SQ_backtest_calc_profit(assest_history):
    return (assest_history[-1] / assest_history[1]) - 1


def SQ_backtest_calc_profit_per_year(assest_history, days):
    return math.pow(float(assest_history[-1]) / float(assest_history[0]), 250.0 / float(days)) - 1.0


def SQ_backtest_calc_profit_matrix(assest_history):
    assest_profit = []
...
def SQ_backtest_calc_volatility(assest_profit_matrix):
    # 策略每日收益的年化标准差
...
def SQ_backtest_calc_dropback_max(history):
    drops = []
...
def SQ_backtest_calc_sharpe(annualized_returns, r, volatility_year):
    '计算夏普比率'
    return (annualized_returns - r) / volatility_year


def SQ_backtest_calc_trade_date(history):
    '计算交易日期'
    trade_date = []
...
def SQ_backtest_calc_win_rate(profit_day):
    # 大于0的次数
    abovez = 0
    belowz = 0
