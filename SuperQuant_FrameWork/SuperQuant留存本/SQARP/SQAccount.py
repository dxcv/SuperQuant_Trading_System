...
from SuperQuant.SQUtil.SQDate_trade import (
...
class SQ_Account(SQ_Worker):
    ...
    @2018/06/11
    SQ_Account不会基于行情计算市值,因此都只会对应记录证券数量和现金资产




    @2018/12/23
    当我们继承/复用 SQ_Account 的时候, 我们需要实现什么
    ...
    def __init__(
        ...
        具体可以参见: https://github.com/SuperQuant/SuperQuant/blob/master/EXAMPLE/test_backtest/FUTURE/TEST_%E4%BF%9D%E8%AF%81%E9%87%91%E8%B4%A6%E6%88%B7.ipynb

        >>>>>>>>>>>>>



        :param [SQ.PARAM] market_type:   市场类别 默认SQ.MARKET_TYPE.STOCK_CN A股股票
        :param [SQ.PARAM] frequence:     账户级别 默认日线SQ.FREQUENCE.DAY
        :param [SQ.PARAM] broker:        BROEKR类 默认回测 SQ.BROKER_TYPE.BACKTEST
        :param [SQ.PARAM] running_environment 当前运行环境 默认Backtest
        ...
        ########################################################################
        # 资产类
        self.orders = SQ_OrderQueue()  # 历史委托单
    ...
    def __repr__(self):
        return '< SQ_Account {} market: {}>'.format(
            self.account_cookie,
            self.market_type
        )
    ...
    def start_date(self):
        ...
        if self.start_ == None:
            if len(self.time_index_max) > 0:
                ...
                print(
                    RuntimeWarning(
                        'SQACCOUNT: THIS ACCOUNT DOESNOT HAVE ANY TRADE'
                    )
                )
        else:
            return self.start_
    ...
    def trade_range(self):
        return SQ_util_get_trade_range(self.start_date, self.end_date)
    ...
    def trade_range_max(self):
        if self.start_date < str(min(self.time_index_max))[0:10]:
            ...
            return SQ_util_get_trade_range(str(min(self.time_index_max))[0:10], str(max(self.time_index_max))[0:10])
    ...
    #             return SQ_util_get_trade_range(self.start_date, self.end_date)
    #        else:
    #            return SQ_util_get_trade_range(str(min(self.time_index))[0:10], str(max(self.time_index))[0:10])
    @property
    ...
    def daily_cash(self):
        ...
        le = pd.DataFrame(
            pd.Series(data=None, index=pd.to_datetime(self.trade_range_max).set_names('date'), name='predrop'))
        ri = res.set_index('date')
        res_ = pd.merge(le, ri, how='left', left_index=True, right_index=True)
        res_ = res_.ffill().fillna(self.init_cash).drop(['predrop', 'datetime', 'account_cookie'],
    ...
    def daily_hold(self):
        ...
        if len(data) < 1:
            ...
            res_ = pd.merge(le, ri, how='left', left_index=True, right_index=True)
            res_ = res_.ffill().fillna(0).drop(['predrop', 'account_cookie'], axis=1).reset_index().set_index(
                ['date']).sort_index()
            res_ = res_[res_.index.isin(self.trade_range)]
            return res_
    ...
    def daily_frozen(self):
        '每日交易结算时的持仓表'
        res_ = self.history_table.assign(date=pd.to_datetime(self.history_table.datetime)).set_index('date').resample(
            'D').frozen.last().fillna(method='pad')
        res_ = res_[res_.index.isin(self.trade_range)]
        return res_
    ...
    def send_order(
        ...
        python code:
        Account=SQ.SQ_Account()

        Order_bymoney=Account.send_order(code='000001',
                                        price=11,
                                        money=0.3*Account.cash_available,
                                        time='2018-05-09',
                                        towards=SQ.ORDER_DIRECTION.BUY,
                                        order_model=SQ.ORDER_MODEL.MARKET,
                                        amount_model=SQ.AMOUNT_MODEL.BY_MONEY
                                        )
        ...
        Order_byamount=Account.send_order(code='000001',
                                        ...
                                        towards=SQ.ORDER_DIRECTION.BUY,
                                        order_model=SQ.ORDER_MODEL.MARKET,
                                        amount_model=SQ.AMOUNT_MODEL.BY_AMOUNT
                                        )
        ...
        :param money: 买卖 价格
        :param order_model: 类型 SQ.ORDER_MODE
        :param amount_model:类型 SQ.AMOUNT_MODEL
        :return:  SQ_Order | False

        @2018/12/23
        send_order 是SQ的标准返回, 如需对接其他接口, 只需要对于SQ_Order做适配即可
        ...
        if int(towards) in [1, 2, 3]:
            ...
            if self.cash_available >= money:
                ...
                wrong_reason = 'SQACCOUNT: 可用资金不足 cash_available {}  code {} time {} amount {} towards {}'.format(
                    self.cash_available,
                    code,
                    time,
                    amount,
        ...
        if flag and (amount > 0):
            ...
            # 历史委托order状态存储， 保存到 SQ_Order 对象中的队列中
            self.datetime = time
    ...
    def close_positions_order(self):
        ...
        if self.running_environment == RUNNING_ENVIRONMENT.TZERO:
            ...
            raise RuntimeError(
                'SQACCOUNT with {} environments cannot use this methods'.format(
                    self.running_environment
                )
            )
    ...
    def settle(self, settle_data=None):
        ...
        self.datetime = '{} 09:30:00'.format(
            SQ_util_get_next_day(self.date)
    ...
    def run(self, event):
        '''
        这个方法是被 SQ_ThreadEngine 处理队列时候调用的， SQ_Task 中 do 方法调用 run （在其它线程中）
       'SQ_WORKER method 重载'
        :param event: 事件类型 SQ_Event
        :return:
        '''
        'SQ_WORKER method'
