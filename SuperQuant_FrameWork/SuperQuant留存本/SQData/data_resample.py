...
def SQ_data_tick_resample_1min(tick, type_='1min', if_drop=True):
    ...
    3. 经测试，可以匹配 SQ.SQ_fetch_get_stock_transaction 得到的数据，其他类型数据未测试
    demo:
    df = SQ.SQ_fetch_get_stock_transaction(package='tdx', code='000001',
                                           start='2018-08-01 09:25:00',
                                           end='2018-08-03 15:00:00')
    df_min = SQ_data_tick_resample_1min(df)
    """
...
def SQ_data_tick_resample(tick, type_='1min'):
    """tick采样成任意级别分钟线
...
def SQ_data_ctptick_resample(tick, type_='1min'):
    """tick采样成任意级别分钟线
...
def SQ_data_min_resample(min_data,  type_='5min'):
    """分钟线采样成大周期
...
def SQ_data_day_resample(day_data,  type_='w'):
    """日线降采样
...
if __name__ == '__main__':
    ...
    tick = SQ.SQ_fetch_get_stock_transaction(
        'tdx', '000001', '2018-08-01', '2018-08-02')
    print(SQ_data_tick_resample(tick, '60min'))
    print(SQ_data_tick_resample(tick, '15min'))
    print(SQ_data_tick_resample(tick, '35min'))
