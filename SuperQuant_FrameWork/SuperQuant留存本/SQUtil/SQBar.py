...
from SuperQuant.SQUtil.SQDate_trade import (SQ_util_date_gap,
                                            ...
                                            SQ_util_if_trade, trade_date_sse)


def SQ_util_make_futuremin_index(day, type_='1min'):
    """创建股票分钟线的index
    ...
    if SQ_util_if_trade(day) is True:
        return pd.date_range(str(day) + '21:00:00', str(day) + '', freq=type_, closed='right').append(
            pd.date_range(str(day) + ' 13:00:00', str(day) + ' 15:00:00', freq=type_, closed='right'))
    else:
        return pd.DataFrame(['No trade'])


def SQ_util_make_min_index(day, type_='1min'):
    """创建股票分钟线的index
...
def SQ_util_make_hour_index(day, type_='1h'):
    """创建股票的小时线的index
...
def SQ_util_time_gap(time, gap, methods, type_):
    '分钟线回测的时候的gap'
    min_len = int(240 / int(str(type_).split('min')[0]))
    day_gap = math.ceil(gap / min_len)

    if methods in ['>', 'gt']:
        data = pd.concat([pd.DataFrame(SQ_util_make_min_index(day, type_)) for day in
                          trade_date_sse[trade_date_sse.index(str(datetime.datetime.strptime(
                              time, '%Y-%m-%d %H:%M:%S').date())):trade_date_sse.index(str(
                              datetime.datetime.strptime(time,
                                                         '%Y-%m-%d %H:%M:%S').date())) + day_gap + 1]]).reset_index()
        ...
        data = pd.concat([pd.DataFrame(SQ_util_make_min_index(day, type_)) for day in
                          trade_date_sse[trade_date_sse.index(str(datetime.datetime.strptime(
                              time, '%Y-%m-%d %H:%M:%S').date())) - day_gap:trade_date_sse.index(
                              str(datetime.datetime.strptime(time, '%Y-%m-%d %H:%M:%S').date())) + 1]]).reset_index()
