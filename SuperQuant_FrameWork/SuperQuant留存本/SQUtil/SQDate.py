...
def SQ_util_today_str():
    """
    返回今天的日期字符串
    :return: 类型字符串 2011-11-11
    """
    dt = SQ_util_date_today()
    return SQ_util_datetime_to_strdate(dt)


def SQ_util_date_str2int(date):
    """
...
def SQ_util_date_int2str(int_date):
    """
    类型datetime.datatime
    :param date: int 8位整数
    :return: 类型str
...
def SQ_util_to_datetime(time):
    ...
    if len(str(time)) == 10:
        ...
        SQ_util_log_info('WRONG DATETIME FORMAT {}'.format(time))
    return datetime.datetime.strptime(_time, '%Y-%m-%d %H:%M:%S')


def SQ_util_datetime_to_strdate(dt):
    """
...
def SQ_util_datetime_to_strdatetime(dt):
    """
    :param dt:  pythone datetime.datetime
...
def SQ_util_pands_timestamp_to_date(pandsTimestamp):
    """
...
def SQ_util_pands_timestamp_to_datetime(pandsTimestamp):
    """
...
def SQ_util_stamp2datetime(timestamp):
    """
    datestamp转datetime
...
def SQ_util_realtime(strtime, client):
    """
    查询数据库中的数据
    :param strtime: strtime  str字符串                 -- 1999-12-11 这种格式
    :param client: client  pymongo.MongoClient类型    -- mongodb 数据库 从 SQ_util_sql_mongo_setting 中 SQ_util_sql_mongo_setting 获取
    :return: Dictionary  -- {'time_real': 时间,'id': id}
    """
    time_stamp = SQ_util_date_stamp(strtime)
...
def SQ_util_id2date(idx, client):
    """
    从数据库中查询 通达信时间
    :param idx: 字符串 -- 数据库index
    :param client: pymongo.MongoClient类型    -- mongodb 数据库 从 SQ_util_sql_mongo_setting 中 SQ_util_sql_mongo_setting 获取
    :return:         Str -- 通达信数据库时间
    """
    coll = client.quantaxis.trade_date
...
def SQ_util_is_trade(date, code, client):
    """
    判断是否是交易日
    从数据库中查询
...
def SQ_util_get_date_index(date, trade_list):
    """
...
def SQ_util_get_index_date(id, trade_list):
    """
    :param id:  ：？？
    :param trade_list:  ？？
    :return: ？？
...
def SQ_util_select_hours(time=None, gt=None, lt=None, gte=None, lte=None):
...
def SQ_util_select_min(time=None, gt=None, lt=None, gte=None, lte=None):
    """
...
def SQ_util_time_delay(time_=0):
    """
...
def SQ_util_calc_time(func, *args, **kwargs):
    """
    '耗时长度的装饰器'
    :param func:
    :param args:
...
if __name__ == '__main__':
    print(SQ_util_time_stamp('2017-01-01 10:25:08'))
