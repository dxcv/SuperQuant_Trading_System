...
#from SuperQuant_CRAWLY.run_selenium_alone import (read_east_money_page_zjlx_to_sqllite, open_chrome_driver, close_chrome_dirver)
from SuperQuant_CRAWLY.run_selenium_alone import *
import urllib
import pandas as pd
import time
...
def SQ_request_eastmoney_zjlx( param_stock_code_list ):
    # 改用
    ...
    for iStockCode in range(len(param_stock_code_list)):
        ...
        # coll_stock_zjlx.insert_many(SQ_util_to_json_from_pandas(df))
...
'''
def SQ_read_eastmoney_zjlx_web_page_to_sqllite(stockCodeList = None):

    # todo 🛠 check stockCode 是否存在有效合法
    # todo 🛠 SQLocalize 从SQLocalize 目录中读取 固定位置存放驱动文件

    print("📨当前工作路径文件位置 : ",os.getcwd())
    path_check = os.getcwd()+"/SuperQuant_WEBDRIVER"
    if os.path.exists(path_check) == False:
        print("😵 确认当前路径是否包含selenium_driver目录 😰 ")
        return
    else:
        print(os.getcwd()+"/SuperQuant_WEBDRIVER"," 目录存在 😁")
    print("")

    # path_for_save_data = SQLocalize.download_path + "/eastmoney_stock_zjlx"
