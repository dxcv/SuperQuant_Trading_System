...
from SuperQuant.SQAnalysis.SQAnalysis_dataframe import SQAnalysis_stock
from SuperQuant.SQData.data_marketvalue import SQ_data_marketvalue
from SuperQuant.SQFetch.Fetcher import SQ_quotation
from SuperQuant.SQFetch.SQQuery import SQ_fetch_stock_info
from SuperQuant.SQFetch.SQQuery_Advance import (SQ_fetch_stock_block_adv,
                                               SQ_fetch_stock_day_adv,
                                               SQ_fetch_stock_min_adv)
from SuperQuant.SQFetch.SQTdx import SQ_fetch_get_stock_info
from SuperQuant.SQFetch.SQTdx_adv import SQ_Tdx_Executor
from SuperQuant.SQUtil.SQDate_trade import SQ_util_get_real_datelist
from SuperQuant.SQUtil.SQParameter import (DATASOURCE, FREQUENCE, MARKET_TYPE,
                                          OUTPUT_FORMAT)


def get_gap_trade(gap):
    return SQ_util_get_real_datelist(datetime.date.today() + datetime.timedelta(days=-int(gap)), datetime.date.today())


#from SuperQuant.SQAnalysis.SQAnalysis_dataframe import SQAnalysis_stock
class SQAnalysis_block():
    ...
    def __repr__(self):
        return '< SQAnalysis_Block {} with {} code >'.format(self.name, len(self.code))

    @property
    @lru_cache()
    def market_data(self):
        return SQ_quotation(self.code, self.start, self.end, self.frequence,
                            market=MARKET_TYPE.STOCK_CN, source=DATASOURCE.MONGO, output=OUTPUT_FORMAT.DATASTRUCT).to_qfq()

    @property
    @lru_cache()
    def market_value(self):
        return self.market_data.add_func(SQ_data_marketvalue)

    @property
    def week_data(self):
        'this weekly data'
        'return a SuperQuant DATASTRUCT'
    ...
    def plot_index(self, methods='mv'):
        ...
        plt.title('SuperQuant BLOCK ANA {}'.format(
...
if __name__ == "__main__":
    ...
    ana = SQAnalysis_block(
        SQ.SQ_fetch_stock_block_adv().get_block('国产软件').code, '国产软件', '2018-01-01', '2018-08-21')
    ana.plot_index()

    ana = SQAnalysis_block(['000001', '000002', '600356'],
