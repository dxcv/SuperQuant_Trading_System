...
SQWind is a data fetch module just for WIND Institution Version

SQWind is under the [SQStandard#0.0.2 @101-1],[SQStandard#0.0.2 @501-0] protocol
...
from SuperQuant.SQUtil import SQ_util_date_valid, SQ_util_log_info

from SuperQuant.SQFetch import data_list as data_list


def SQ_fetch_get_stock_info(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
    w.start()
    # get the all stock list on the endDate
    # judge the vaild date
    if(SQ_util_date_valid(endDate) is False):
        SQ_util_log_info("wrong date")
    else:
        ...
        data = w.wsd(name, "sec_name,sec_englishname,ipo_date,exch_city,mkt,\
                    ...
                    parallelcode,sec_type,backdoor,backdoordate,windtype",
                     startDate, endDate)
        # SQ_util_log_info(data)
        if (data.ErrorCode != 0):
            SQ_util_log_info("Connect to Wind successfully")
            return data.Data


def SQ_fetch_get_stock_day(name, startDate, endDate, if_fq='01'):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
    w.start()
    if(SQ_util_date_valid(endDate) == False):
        SQ_util_log_info("wrong date")
    else:
        if if_fq in ['00', 'bfq']:
            ...
            SQ_util_log_info('wrong fq factor! using qfq')
            data = w.wsd(name, "sec_name,pre_close,open,high,low,close,volume",
                         startDate, endDate, "PriceAdj=B")
        if (data.ErrorCode == 0):
            SQ_util_log_info("Connect to Wind successfully")
...
def SQ_fetch_get_stock_day_simple(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
    ...
    if(SQ_util_date_valid(endDate) == False):
        ...
        SQ_util_log_info(data.ErrorCode)
        if (data.ErrorCode == 0):
            SQ_util_log_info("Connect to Wind successfully")
            return data.Data


def SQ_fetch_get_stock_indicator(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
...
def SQ_fetch_get_stock_shape(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
...
def SQ_fetch_get_stock_risk(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
...
def SQ_fetch_get_stock_xueqiu(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
...
def SQ_fetch_get_stock_financial(name, startDate, endDate):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
...
def SQ_fetch_get_trade_date(endDate, exchange):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
    ...
    if (exchange in supportExchanges):
        ...
        # SQ_util_log_info(data.Data)
        dates = pd.DataFrame(np.asarray(data.Data).T,
                             columns=data.Fields, index=data.Times)
    else:
        SQ_util_log_info("exchange name problem")
    return dates


def SQ_fetch_get_stock_list(date):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
    w.start()
    if(SQ_util_date_valid(date) == False):
        SQ_util_log_info("wrong date")
    else:
...
def SQ_fetch_get_stock_list_special(date, id):
    try:
        from WindPy import w
    except:
        SQ_util_log_info('No WindPY Module!')
