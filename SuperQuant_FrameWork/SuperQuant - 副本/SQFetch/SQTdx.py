...
#from SuperQuant.SQData.data_fq import SQ_data_make_qfq, SQ_data_make_hfq
...
from SuperQuant.SQUtil import (SQ_Setting, SQ_util_date_stamp,
                              SQ_util_date_str2int, SQ_util_date_valid,
                              SQ_util_get_real_date, SQ_util_get_real_datelist,
                              SQ_util_future_to_realdatetime, SQ_util_future_to_tradedatetime,
                              SQ_util_get_trade_gap, SQ_util_log_info,
                              SQ_util_time_stamp, SQ_util_web_ping,
                              exclude_from_stock_ip_list, future_ip_list,
                              stock_ip_list, trade_date_sse)
from SuperQuant.SQUtil.SQSetting import SQSETTING
...
def ping(ip, port=7709, type_='stock'):
    ...
    try:
        ...
        if isinstance(e, TypeError):
            print(e)
            print('Tushare内置的pytdx版本和SuperQuant使用的pytdx 版本不同, 请重新安装pytdx以解决此问题')
...
def select_best_ip():
    SQ_util_log_info('Selecting the Best Server IP of TDX')
...
def SQ_fetch_get_stock_day(code, start_date, end_date, if_fq='00', frequence='day', ip=None, port=None):
    ...
    try:
        with api.connect(ip, port, time_out=0.7):
            ...
            lens = SQ_util_get_trade_gap(start_date, today_)
            ...
            data = data.assign(date=data['datetime'].apply(lambda x: str(x[0:10])),
                               code=str(code),
                               date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(str(x)[0:10])))\
            ...
            if if_fq in ['00', 'bfq']:
                ...
                return None
                # xdxr = SQ_fetch_get_stock_xdxr(code)
                # if if_fq in ['01','qfq']:
                #     return SQ_data_make_qfq(data,xdxr)
                # elif if_fq in ['02','hfq']:
                #     return SQ_data_make_hfq(data,xdxr)
    except Exception as e:
        if isinstance(e, TypeError):
            print('Tushare内置的pytdx版本和SuperQuant使用的pytdx 版本不同, 请重新安装pytdx以解决此问题')
            print('pip uninstall pytdx')
...
def SQ_fetch_get_bond_day(code, start_date, end_date, frequence='day', ip=None, port=None):
    ...
    with api.connect(ip, port):
        ...
        data = data.assign(date=data['datetime'].apply(lambda x: str(x[0:10]))).assign(code=str(code))\
            .assign(date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(str(x)[0:10])))\
            .set_index('date', drop=False, inplace=False)\
            .assign(code=code)\
...
def SQ_fetch_get_index_min(code, start, end, frequence='1min', ip=None, port=None):
    ...
    with api.connect(ip, port):
        ...
        data = data\
            ...
            .assign(date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(x)))\
            .assign(time_stamp=data['datetime'].apply(lambda x: SQ_util_time_stamp(x)))\
...
def __SQ_fetch_get_stock_transaction(code, day, retry, api):
    ...
    while cur_offset <= max_offset:
        one_chunk = api.get_history_transaction_data(
            _select_market_code(str(code)), str(code), cur_offset * batch_size, batch_size, SQ_util_date_str2int(day))
    ...
    for _ in range(retry):
        if len(data_) < 2:
            return __SQ_fetch_get_stock_transaction(code, day, 0, api)
        else:
...
def SQ_fetch_get_stock_transaction(code, start, end, retry=2, ip=None, port=None):
    ...
    real_start, real_end = SQ_util_get_real_datelist(start, end)
    ...
    with api.connect(ip, port):
        ...
        for index_ in range(trade_date_sse.index(real_start), trade_date_sse.index(real_end) + 1):
            ...
            try:
                data_ = __SQ_fetch_get_stock_transaction(
                    code, trade_date_sse[index_], retry, api)
                if len(data_) < 1:
                    return None
            except:
                SQ_util_log_info('Wrong in Getting {} history transaction data in day {}'.format(
                    code, trade_date_sse[index_]))
            else:
                SQ_util_log_info('Successfully Getting {} history transaction data in day {}'.format(
                    code, trade_date_sse[index_]))
...
def SQ_fetch_get_stock_block(ip=None, port=None):
    ...
    with api.connect(ip, port):
        ...
        if len(data) > 10:
            ...
            SQ_util_log_info('Wrong with fetch block ')
...
def SQ_fetch_get_extensionmarket_count(ip=None, port=None):
...
def SQ_fetch_get_extensionmarket_info(ip=None, port=None):
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    with apix.connect(ip, port):
        global extension_market_info
...
def SQ_fetch_get_extensionmarket_list(ip=None, port=None):
    '期货代码list'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    with apix.connect(ip, port):
...
def SQ_fetch_get_future_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_goods_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_globalfuture_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_hkstock_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_hkindex_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_hkfund_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_usstock_list(ip=None, port=None):
    """[summary]
...
def SQ_fetch_get_macroindex_list(ip=None, port=None):
    """宏观指标列表
...
def SQ_fetch_get_option_list(ip=None, port=None):
    """期权列表
...
def SQ_fetch_get_50etf_option_contract_time_to_market():
    '''
    #🛠todo 获取期权合约的上市日期 ？ 暂时没有。
    去掉商品期权，保留510050开头的50ETF期权,只获取50ETF期权
    :return: list Series
    '''
    result = SQ_fetch_get_option_list('tdx')
...
def SQ_fetch_get_commodity_option_CU_contract_time_to_market():
    '''
    #🛠todo 获取期权合约的上市日期 ？ 暂时没有。
    去掉商品期权，保留510050开头的50ETF期权,只获取50ETF期权
    :return: list Series
...
def SQ_fetch_get_commodity_option_M_contract_time_to_market():
    '''
    #🛠todo 获取期权合约的上市日期 ？ 暂时没有。
    去掉商品期权，保留510050开头的50ETF期权,只获取50ETF期权
    :return: list Series
...
def SQ_fetch_get_commodity_option_SR_contract_time_to_market():
    '''
    #🛠todo 获取期权合约的上市日期 ？ 暂时没有。
    去掉商品期权，保留510050开头的50ETF期权,只获取50ETF期权
    :return: list Series
...
def SQ_fetch_get_exchangerate_list(ip=None, port=None):
    """汇率列表
...
def SQ_fetch_get_future_day(code, start_date, end_date, frequence='day', ip=None, port=None):
    '期货数据 日线'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    start_date = str(start_date)[0:10]
    ...
    with apix.connect(ip, port):
        ...
        try:
            ...
            #获取商品期货 的适合 会报None
            data = data.assign(date=data['datetime'].apply(lambda x: str(x[0:10]))).assign(code=str(code))\
                .assign(date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(str(x)[0:10]))).set_index('date', drop=False, inplace=False)
...
def SQ_fetch_get_future_min(code, start, end, frequence='1min', ip=None, port=None):
    '期货数据 分钟线'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    type_ = ''
    ...
    with apix.connect(ip, port):
        ...
        data = data\
            ...
            .assign(datetime=pd.to_datetime(data['datetime'].apply(SQ_util_future_to_realdatetime, 1)))\
            .drop(['year', 'month', 'day', 'hour', 'minute'], axis=1, inplace=False)\
            .assign(date=data['datetime'].apply(lambda x: str(x)[0:10]))\
            .assign(date_stamp=data['datetime'].apply(lambda x: SQ_util_date_stamp(x)))\
            .assign(time_stamp=data['datetime'].apply(lambda x: SQ_util_time_stamp(x)))\
...
def __SQ_fetch_get_future_transaction(code, day, retry, code_market, apix):
    ...
    while cur_offset <= max_offset:
        one_chunk = apix.get_history_transaction_data(
            code_market, str(code), SQ_util_date_str2int(day), cur_offset * batch_size)
    ...
    for _ in range(retry):
        if len(data_) < 2:
            return __SQ_fetch_get_stock_transaction(code, day, 0, apix)
        else:
            return data_.assign(datetime=pd.to_datetime(data_['date'])).assign(date=str(day))\
                        .assign(code=str(code)).assign(order=range(len(data_.index))).set_index('datetime', drop=False, inplace=False)


def SQ_fetch_get_future_transaction(code, start, end, retry=2, ip=None, port=None):
    '期货历史成交分笔'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    global extension_market_list
    ...
    with apix.connect(ip, port):
        ...
        for index_ in range(trade_date_sse.index(real_start), trade_date_sse.index(real_end) + 1):
            ...
            try:
                data_ = __SQ_fetch_get_future_transaction(
...
def SQ_fetch_get_future_transaction_realtime(code, ip=None, port=None):
    '期货历史成交分笔'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    global extension_market_list
...
def SQ_fetch_get_future_realtime(code, ip=None, port=None):
    '期货实时价格'
    ip, port = get_extensionmarket_ip(ip, port)
    apix = TdxExHq_API()
    global extension_market_list
...
def SQ_fetch_get_wholemarket_list():
    hq_codelist = SQ_fetch_get_stock_list(
        type_='all').loc[:, ['code', 'name']].set_index(['code', 'name'], drop=False)
    kz_codelist = SQ_fetch_get_extensionmarket_list().loc[:, ['code', 'name']].set_index([
...
if __name__ == '__main__':
    rows = SQ_fetch_get_commodity_option_CU_contract_time_to_market()
    print(rows)

    print(SQ_fetch_get_stock_day('000001', '2017-07-03', '2017-07-10'))
    print(SQ_fetch_get_stock_day('000001', '2013-07-01', '2013-07-09'))
    # print(SQ_fetch_get_stock_realtime('000001'))
    #print(SQ_fetch_get_index_day('000001', '2017-01-01', '2017-07-01'))
    # print(SQ_fetch_get_stock_transaction('000001', '2017-07-03', '2017-07-10'))

    # print(SQ_fetch_get_stock_info('600116'))
